  ...
  const getData = useCallback(async () => {
    try {
      const response = await fetch(
        `https://backend-jazee.onrender.com/recuiter/sort/student/?page=${page}`,
        {
          headers: {
            'Content-Type': 'application/json',
          },
...
console.log(maxPage);
  ...
  const applicationHandler = async (event, comp) => {
    ...
    try {
      const response = await fetch(
        `https://backend-jazee.onrender.com/recuiter/save/student/?rec=${comp.username}&student=${username}`,
        { method: 'GET' } 
      );
      const resp = await response.json();
      alert(resp.message);
